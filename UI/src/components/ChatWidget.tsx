...
function renderInlineMarkdown(text: string): ReactNode[] {
  ...
  return parts.map((part, idx) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) {
      return (
        <code key={idx} className="rounded bg-slate-800/80 px-1.5 py-0.5 text-[0.9em] text-cyan-200">
          {part.slice(1, -1)}
        </code>
      );
    }
...
function renderMarkdown(text: string): ReactNode[] {
  ...
  while (i < lines.length) {
    ...
    if (codeFence) {
      ...
      blocks.push(
        <pre key={`code-${i}`} className="my-2 overflow-x-auto rounded-lg border border-slate-700 bg-slate-900 p-3 text-xs text-slate-200">
          {language ? <div className="mb-2 text-[10px] uppercase tracking-wide text-slate-400">{language}</div> : null}
          <code>{codeLines.join('\n')}</code>
        </pre>
      );
      continue;
    ...
    if (heading) {
      ...
      blocks.push(
        <p key={`h-${i}`} className={`${cls} mt-1 mb-1.5 text-slate-100`}>
    ...
    if (/^>\s+/.test(line)) {
      ...
      blocks.push(
        <blockquote key={`q-${i}`} className="my-2 border-l-2 border-cyan-400/40 pl-3 text-slate-300">
          {quotes.map((q, idx) => (
            <p key={idx} className={idx > 0 ? 'mt-1' : ''}>{renderInlineMarkdown(q)}</p>
          ))}
        </blockquote>
      );
      continue;
